...
export type GuidanceResult = {
  lines: string[];
  mode?: 'insight' | 'action';
  type?: 'inner' | 'outer' | 'mixed';
};

export const fetchGuidance = async (
  language: TargetLanguage, 
  prompt?: string,
  targetMode?: 'insight' | 'action'
): Promise<GuidanceResult> => {
  // Always try to fetch from the real API first
  try {
    ...
    // If targetMode is explicitly provided, send it to force that engine
    if (targetMode === 'insight' || targetMode === 'action') {
      requestBody.targetMode = targetMode;
    }
    ...
    if (response.ok) {
      ...
      if (data.lines && data.lines.length > 0) {
        return {
          lines: data.lines,
          mode: data.mode || 'insight',
          type: data.type,
        };
      } else {
        console.error('API returned empty lines:', data);
        return {
          lines: ["I'm here, but something went wrong. Please try again."],
          mode: 'insight',
        };
      }
    } else {
      ...
      return {
        lines: [`I'm having trouble connecting right now. Error: ${response.status}. Please check your API configuration.`],
        mode: 'insight',
      };
    }
  } catch (error) {
    ...
    return {
      lines: [`I'm having trouble connecting right now. Please check your API configuration. Error: ${error instanceof Error ? error.message : 'Unknown error'}`],
      mode: 'insight',
    };
  }
};
